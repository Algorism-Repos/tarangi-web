...
// const OrdersSection = ({ orders, setShowOrderPopup, setSelectedOrder }) => {
//   function convertOrders(rawOrders) {
//     return rawOrders.map((order) => {
//       const hasFulfillments = order.fulfillments?.length > 0;

//       const items = hasFulfillments
//         ? order.fulfillments.flatMap((f) => f.line_items)
//         : order.line_items;
...
//     });
//   }
//   const formatted = convertOrders(orders);
//   const upcomingOrders = formatted.filter((order) => !order.isFulfilled);
//   const deliveredOrders = formatted.filter(
